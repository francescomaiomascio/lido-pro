<script lang="ts">
  ...
  let { vm, activeLayers }: { vm: MetricBoardViewModel; activeLayers: MapStudioLayerId[] } = $props()
  const sandActive = $derived(activeLayers.includes('sand.surface') || activeLayers.includes('base.surface'))
  const seaActive = $derived(activeLayers.includes('sea.edge'))
</script>

<g class="parametric-board__surface" class:is-muted={!sandActive && !seaActive} filter="url(#board-shadow)">
  <rect x={vm.frame.frameX} y={vm.frame.frameY} width={vm.frame.frameW} height={vm.frame.frameH} rx="22" fill="url(#board-sand)" />
